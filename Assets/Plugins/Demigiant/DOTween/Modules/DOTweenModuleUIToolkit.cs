...
namespace DG.Tweening
{
    ...
    {
        private static Vector3 GetTranslate(VisualElement target)
        {
            return target.resolvedStyle.translate;
        }

        private static void SetTranslate(VisualElement target, Vector3 value)
        {
            target.style.translate = new Translate(value.x, value.y, value.z);
        }
        ...
        public static TweenerCore<Vector3, Vector3, VectorOptions> DOMove(this VisualElement target, Vector3 endValue, float duration, bool snapping = false)
        {
            TweenerCore<Vector3, Vector3, VectorOptions> t
                = DOTween.To(() => GetTranslate(target), x => SetTranslate(target, x), endValue, duration);
        ...
        public static TweenerCore<Vector2, Vector2, VectorOptions> DOMove(this VisualElement target, Vector2 endValue, float duration, bool snapping = false)
        {
            TweenerCore<Vector2, Vector2, VectorOptions> t
                = DOTween.To(
                    () =>
                    {
                        Vector3 currentTranslate = GetTranslate(target);
                        return new Vector2(currentTranslate.x, currentTranslate.y);
                    },
                    x => SetTranslate(target, new Vector3(x.x, x.y, 0f)),
                    endValue,
                    duration
                );
            t.SetOptions(snapping).SetTarget(target);
        ...
        {
            TweenerCore<Vector3, Vector3, VectorOptions> t
                = DOTween.To(() => GetTranslate(target), x => SetTranslate(target, x), new Vector3(endValue, 0, 0), duration);
        ...
        {
            TweenerCore<Vector3, Vector3, VectorOptions> t
                = DOTween.To(() => GetTranslate(target), x => SetTranslate(target, x), new Vector3(0, endValue, 0), duration);
        ...
        {
            TweenerCore<Vector3, Vector3, VectorOptions> t
                = DOTween.To(() => GetTranslate(target), x => SetTranslate(target, x), new Vector3(0, 0, endValue), duration);
